...
exports.create = async (req, res) => {
    const { userID, date, description, amount, category, paymentMethod } = req.body; //คือการสลายโครงสร้าง เพื่อให้เรียกใช้ข้อมูลได้ง่ายๆ
    const newRecord = {
        userID,
        date,
        description,
    ...
    await Financial.create(newRecord).then((data) => {      //then คือการ
        res.send(data);
    }).catch((error) => {
        res.status(500).send({
            message:
